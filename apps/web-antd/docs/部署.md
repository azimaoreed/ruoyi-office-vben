...
**错误示例**:
- 请求路径错误：`http://182.92.106.252/web/admin-api/...`（错误，多了 `/web` 前缀）
- 正确路径应该是：`http://182.92.106.252/admin-api/...`

**常见原因**:

1. **`.env.production` 配置错误**：`VITE_BASE_URL` 设置为 `/web`
...
1. **检查浏览器网络面板**：
   - 打开开发者工具 -> Network
   - 查看 API 请求的 URL
   - 应该是：`http://182.92.106.252/admin-api/...`（不是 `/web/admin-api/...`）

2. **测试代理是否工作**：
   ```bash
   # 在服务器上测试
   curl http://localhost/admin-api/system/tenant/get-by-website?website=182.92.106.252
   ```

3. **检查后端服务**：
   ```bash
   # 确认后端服务正常运行
   curl http://192.168.30.130:48080/admin-api/system/tenant/get-by-website?website=182.92.106.252
   ```
