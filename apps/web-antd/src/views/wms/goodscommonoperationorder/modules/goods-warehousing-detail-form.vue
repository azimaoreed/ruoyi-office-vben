...
const onDelete =  async (row: GoodsCommonOperationOrderApi.GoodsWarehousingDetail) => {
...
watch(
    ...
    async (val) => {
      ...
      await nextTick();
      await gridApi.grid.loadData(await getGoodsWarehousingDetailListByCommonOperationId(props.commonOperationId!));
    },
    { immediate: true },
);
</script>
...
<template>
  ...
  <div class="flex justify-center -mt-4">
    <Button :icon="h(Plus)" type="primary" ghost @click="onAdd" v-access:code="['wms:goods-common-operation-order:create']">
