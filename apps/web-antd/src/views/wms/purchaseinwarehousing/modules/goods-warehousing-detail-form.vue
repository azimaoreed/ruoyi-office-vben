...
const onDelete =  async (row: PurchaseInWarehousingApi.GoodsWarehousingDetail) => {
...
defineExpose({
  getData: (): PurchaseInWarehousingApi.GoodsWarehousingDetail[] => {
    const data = gridApi.grid.getData() as PurchaseInWarehousingApi.GoodsWarehousingDetail[];
    const removeRecords = gridApi.grid.getRemoveRecords() as PurchaseInWarehousingApi.GoodsWarehousingDetail[];
    const insertRecords = gridApi.grid.getInsertRecords() as PurchaseInWarehousingApi.GoodsWarehousingDetail[];
    return data
...
watch(
    ...
    async (val) => {
      ...
      await nextTick();
      await gridApi.grid.loadData(await getGoodsWarehousingDetailListByPurchaseOrderId(props.purchaseOrderId!));
    },
    { immediate: true },
);
</script>
...
<template>
  ...
  <div class="flex justify-center -mt-4">
    <Button :icon="h(Plus)" type="primary" ghost @click="onAdd" v-access:code="['wms:purchase-in-warehousing:create']">
